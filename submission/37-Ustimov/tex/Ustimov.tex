...
For the system deployed in this study, calibration performed on May~29, 2023 yielded a rotation submatrix close to identity, confirming near-parallel physical alignment of the cameras~\cite{3,4}. However, calibration stability is limited in field conditions. Repeat calibrations on different days revealed disparity shifts of up to 30 pixels for the same object, equivalent to $\approx 900$~m in height. The observed drift is most plausibly attributed to diurnal temperature variations and wind-induced mechanical deformations of the camera mount, and it motivates regular recalibration before each observation session.
