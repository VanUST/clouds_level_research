...
import numpy as np
import cv2
from feature_matching import get_corresponding_feature_points_loftr, cluster_vecs, apply_affine_matrix
...
def find_distance_by_image_pair(img1_path,img2_path,M, base, angle_of_view, orig_size,target_size, hdbscan_config, match_tresh = 0.5):
        ...
        kp1, kp2 = get_corresponding_feature_points_loftr(img1, img2, vizualize=True, match_tresh=match_tresh)

        # Calculate shift array and cluster centers
        shift_arr = kp1 - kp2
        centers = cluster_vecs(shift_arr=shift_arr, hdbscan_config=hdbscan_config, show_clusters=True)

        # Calculate and print distances for each cluster center
        for center in centers:
            distance = count_dist(base, angle_of_view, orig_size[0], center[0])
            print(f"Distance for {img1_path}: {distance}")
...
# Parameters
directory = r"D:\clouds_research\data\test_images\20170904_1050"
base = 17.0
angle_of_view = 62.0
width = 3072.0
orig_size = (3072,2304)
...
config = {'min_cluster_size': 3, 'min_samples': 3, 'epsilon': 3}

# Process image pairs
process_image_pairs(directory, base, angle_of_view, orig_size, target_size, M, config, 0.5)
