...
# Example image pairs (UPDATE THESE PATHS)
if __name__ == "__main__":
    image_pairs = [
        # (r'path/to/image_A_1.jpg', r'path/to/image_A_2.jpg'),
        # (r'path/to/image_B_1.jpg', r'path/to/image_B_2.jpg'),
    ]
    
    if not image_pairs:
        print("Please update the 'image_pairs' list in this script with paths to your calibration images.")
    else:
        M = affine_calibration(image_pairs)
        if M is not None:
            print("\nCopy the 2x3 matrix below into your config.py file as AFFINE_MATRIX:")
            print(repr(M[:2,:]))